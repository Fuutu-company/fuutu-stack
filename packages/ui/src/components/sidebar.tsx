	...
	(
		...
		const setOpen = React.useCallback(
			(value: boolean | ((value: boolean) => boolean)) => {
				...
				// Persist sidebar state in a cookie so SSR can read it back.
				// `document.cookie` is the simplest persistence for this UI-only
				// preference; a full Cookie Store abstraction would be overkill.
				if (typeof document !== "undefined") {
					// biome-ignore lint/suspicious/noDocumentCookie: UI-only preference flag
...
>(({ className, showIcon = false, ...props }, ref) => {
	// Deterministic width to avoid hydration mismatch — use useId as seed.
	// Random width between 50 to 90%.
	const reactId = React.useId();
	const width = React.useMemo(() => {
		const seed = reactId.charCodeAt(0) + reactId.charCodeAt(1);
		return `${(seed % 40) + 50}%`;
	}, [reactId]);
